// Shrimpy changed websocket feed to private, so public code no longer works.
...
upd:{
  ...
  if[`channel in key j;
    if[j[`channel] ~ "trade";                                                     //check for handler of this message type
      content:0N! j[`content];
      contentcount:count content;
      `trades insert (ex:contentcount#`$j[`exchange]; sym:contentcount#`$j[`pair]; time:"P"$content[`time];price:"F"$content[`price];side:`$content[`takerSide];size:"F"$content[`quantity]);
    ];
  ];
 }
