...
VERBOSE:@[value;`.reqnew.VERBOSE;$[count .z.x;"-verbose" in .z.x;0b]];                 //default to non-verbose output
...
send:{[m;u;hd;p;v]
  q:@[.reqnew.query;`method`url`headers`body;:;(m;.url.parse0[0]u;hd;p)];              //parse URL into URL object & build query
...
.reqnew.get:{parseresp okstatus[.reqnew.VERBOSE] send[`GET;x;y;();.reqnew.VERBOSE]}
...
// @return {(dict;string)|any} HTTP response (headers;body), or parsed if JSON
.reqnew.post:{parseresp okstatus[.reqnew.VERBOSE] send[`POST;x;y;z;.reqnew.VERBOSE]}
...
// @return {(dict;string)|any} HTTP response (headers;body), or parsed if JSON
// @qlintsuppress RESERVED_NAME
.reqnew.delete:{parseresp okstatus[.reqnew.VERBOSE] send[`DELETE;x;y;z;.reqnew.VERBOSE]}
